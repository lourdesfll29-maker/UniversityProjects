...
%     frecuencia de y2_sinruido con y2).                                  %
...
%     con los espectros en frecuencias de las señales y2_sinruido e y2.   %
%     Según el ruido eliminado, considerar la opción de probar una nueva  %
%     frecuencia de corte.                                                %
%   - Obtener el diagrama de Bode del filtro diseñado.                    %
%   - Dibujar las señales y2, y2_sinruido e y2_filtrada en el dominio del %
%     tiempo.                                                             %
% Este filtro bilineal debe ser distinto al empleado en el Filtro_01.m    %
% por lo que se ha optado por un filtro paso alto/bajo/banda bilineal.    %
% *********************************************************************** %
% Autores/as - Correo:                                                    %
%                                                                         %
...
% Parámetros fijos
% --------------------------------- %
T = T2;            % Periodo de muestreo de la señal
wS = (pi * 2) / T; % Frecuencia de muestreo (Hz)
y = y2;            % Array con la señal discreta
tt = tt2;          % Array con los tiempos
% --------------------------------- %
% wC propuesta
% --------------------------------- %
wC_low = 14;             % Frecuencia de corte para paso bajo (Hz) 
wC_rechazo = [14.4, 24]; % Banda de rechazo (Hz)
% --------------------------------- %
...
% --------------------------------- %
Y = abs(fft(y2));
Y_sinruido = abs(fft(y2_sinruido));
...
% ----------------------------------------------------------------------- %
% Rechazo banda proporciona el mejor resultado.
% --------------------------------- %
figure;
hold on;
plot(tt, y, 'b', 'LineWidth', 1.2); 
plot(tt, y2_sinruido, 'g', 'LineWidth', 1.2);
plot(tt, y_filtrada_rechazo, 'r', 'LineWidth', 1.5); 
hold off;
legend('Señal original', 'Señal sin ruido', 'Señal filtrada rechazo banda')
xlabel('Tiempo (s)')
ylabel('Amplitud')
